...
export default function SorteioTimes({ adminData, sharedLocations = [] }: Props) {
  ...
  useEffect(() => {
    ...
    if (isAutoPlaying && isAnimatedDrawActive && !isShuffling) {
      if (currentStepIndex < drawSequence.length - 1) {
        timer = setTimeout(() => {
          handleNextStep();
        }, 2200);
  ...
  }, [isAutoPlaying, isAnimatedDrawActive, isShuffling, currentStepIndex, drawSequence.length]);
      ...
      <AnimatePresence>
        {isAnimatedDrawActive && drawSequence.length > 0 && (
          ...
          >
            ...
            <div className="w-full max-w-6xl flex items-center justify-between bg-slate-900/90 border border-slate-800 p-4 rounded-3xl shadow-2xl">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-emerald-400 font-bold uppercase tracking-wider">
                    Sorteando 1 atleta por vez dos potes • Escolha 100% Aleatória
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3.5 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 rounded-xl text-xs font-black uppercase tracking-wider transition flex items-center gap-1.5"
                >
                  <FastForward className="w-4 h-4 text-primary-yellow" /> Revelar Tudo
                </button>
            ...
            {/* Main Stage Arena Box */}
            <div className="w-full max-w-3xl my-6 space-y-6 text-center">
              {/* Progress Indicator */}
              <div className="space-y-2">
              ...
              {/* Big Player Reveal Card (Fut Card Style) */}
              <motion.div
                key={currentStepIndex}
                initial={{ scale: 0.9, opacity: 0 }}
                animate={{ scale: 1, opacity: 1 }}
                className="bg-gradient-to-b from-slate-900 to-slate-950 border-2 border-emerald-500/60 p-6 sm:p-8 rounded-3xl shadow-2xl space-y-4 relative overflow-hidden flex flex-col items-center justify-center"
              >
                ...
                <p className="text-xs font-black uppercase tracking-widest text-emerald-400">
                  {isShuffling ? 'Sorteando no Pote...' : 'Jogador Sorteado!'}
                </p>
                ...
                {!isShuffling && drawSequence[currentStepIndex] && (
                  <motion.div
                    ...
                    className="inline-flex items-center gap-3 bg-emerald-500/20 border-2 border-emerald-500/60 text-emerald-300 px-6 py-3 rounded-2xl text-sm sm:text-base font-black uppercase italic shadow-lg"
                  >
                    <span>Destino:</span>
                ...
                <div className="pt-4 border-t border-slate-800 flex items-center justify-center gap-3 flex-wrap">
                  <button
                    ...
                    className="p-3 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-2xl disabled:opacity-30 transition border border-slate-700"
                    title="Anterior"
                  >
                    <ArrowLeft className="w-5 h-5" />
                  </button>
                  ...
                  <button
                    ...
                    className={`px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-wider transition flex items-center gap-2 shadow-lg ${
                      isAutoPlaying
                  ...
                  >
                    ...
                    {isAutoPlaying ? 'Pausar Sorteio' : 'Reproduzir Automático'}
                  </button>
                  ...
                  <button
                    ...
                    className="px-6 py-3 bg-slate-800 hover:bg-slate-700 text-white rounded-2xl font-black text-xs uppercase tracking-wider transition flex items-center gap-2 border border-slate-700 disabled:opacity-30"
                  >
                    Próximo Atleta <SkipForward className="w-4 h-4 text-primary-yellow" />
                  </button>
                </div>
              </motion.div>
            </div>

            {/* Live Fill-up Teams Grid */}
            <div className="w-full max-w-6xl space-y-3">
              <p className="text-xs font-black uppercase text-slate-400 text-center tracking-widest">
                Preenchimento de Vagas nos Times
              </p>
                ...
                {currentStepTeams.map(t => (
                  <div key={t.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-3 space-y-2">
                    <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                      <span className="font-black text-xs text-white uppercase italic">{t.name}</span>
